...
The Scenario Lab is a deterministic annual cash-flow identity, explicitly distinct from the validated forecast models. It is **anchored at the last complete fiscal year**: debt (Treasury annual record), GDP, population, CPI, and OMB receipts, net outlays, and net interest outlays all share that one fiscal-year-end vintage — mixing today's debt with last year's flows would re-count borrowing already inside the debt figure, so the latest daily balance is shown as context but never simulated. The outlay split uses **actual interest paid** (FYOINT): primary outlays = net outlays − interest outlays, and the interest-rate slider defaults to the observed effective rate (interest ÷ debt, ≈2.6% for FY2025). Per simulated fiscal year: interest = rate × start-of-year debt; deficit = primary outlays + interest − receipts; surpluses reduce debt, and a fully repaid debt floors at zero rather than compounding into negative debt. Users set real growth rates plus inflation (deflation allowed); nominal paths compound both ((1+real)(1+inflation)−1), and real outputs are restated in 2025 dollars by deflating cumulative scenario inflation from the anchor CPI. Nothing is hard-coded.
