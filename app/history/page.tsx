...
import { majorMarketShocks } from "@/lib/market-shocks";
...
export default function Page() {
  ...
  return <main className="wrap">
    ...
    <div className="panel"><div className="panelHeader chartNarrative"><div><h2>How did the national debt evolve?</h2><p>Drag across the chart, enter exact dates, or choose a documented shock window to compare fiscal movement.</p></div><span className="pill">Nominal USD · linear</span></div><LineChart intervalControls shocks={majorMarketShocks} values={observations.map((point) => point.debt)} realValues={real.map((point) => point.debt)} labels={observations.map((point) => point.date.slice(0, 7))} dates={observations.map((point) => point.date)} xLabel="Calendar year" yLabel="Total public debt · nominal USD" markers={markers}/></div>
